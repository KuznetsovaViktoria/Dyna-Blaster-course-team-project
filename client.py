    ...
    def draw(self):
        ...
        for obj in objects:
            if obj.type == 'tank':
                ...
                if i == 0:
                    rect = text.get_rect(left=8, centery=TILE // 2 + 0 * TILE)
                elif i == 1:
                    rect = text.get_rect(right=WIDTH - 8, centery=TILE // 2 + 0 * TILE)
                elif i == 2:
                    rect = text.get_rect(left=8, centery=TILE // 2 + 1 * TILE)
                elif i == 3:
                    rect = text.get_rect(right=WIDTH - 8, centery=TILE // 2 + 1 * TILE)
                elif i == 4:
                    rect = text.get_rect(left=8, centery=TILE // 2 + 2 * TILE)
                elif i == 5:
                    rect = text.get_rect(right=WIDTH - 8, centery=TILE // 2 + 2 * TILE)
                elif i == 6:
                    rect = text.get_rect(left=8, centery=TILE // 2 + 3 * TILE)
                elif i == 7:
                    rect = text.get_rect(right=WIDTH - 8, centery=TILE // 2 + 3 * TILE)
                else:
                    rect = text.get_rect()
...
class MyTank:
    ...
    def update(self):
        ...
        if keys[self.keyLEFT]:
            ...
            self.rect.y = max(POINTS_HEIGHT, self.rect.y - self.moveSpeed)
...
while play:
    ...
    if GAME_STARTED and not GAME_FINISHED:
        ...
        for tileX in range(0, WIDTH, TILE):
            for tileY in range(POINTS_HEIGHT, HEIGHT, TILE):
